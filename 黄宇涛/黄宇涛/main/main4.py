...
class Music_Button():
    def __init__(self, music_name, center_y, color, image_area):
        super().__init__()
...
for i in range(len(音乐总和_name)):
    ...
    音乐按钮总和.append(Music_Button(音乐总和_name[i], 65 * i + 35, color_music, music_area))
...
def 听音乐screen():
    ...
    music_area.update()
    # pygame.draw.rect(music_area.image_area, (255, 0, 255), (0, 0, music_area.rect_area.width, music_area.rect_area.height), border_radius=30)
    for i in 音乐按钮总和:
        i.update()
    # 播放区
