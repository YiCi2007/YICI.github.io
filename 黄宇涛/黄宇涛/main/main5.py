...
class Button(Underline, pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        pygame.draw.rect(self.image, (255, 0, 0),
                         ((self.width - self.line_width) / 2, self.height - 10, self.line_width, 10), border_radius=4)
        screen.blit(self.image, self.rect)
...
class Music_Area(Music_Panel):
    def __init__(self):
        ...
        self.music_end_text = self.font.render("到底了~~~", True, (240, 240, 240))
    def update(self):
        global 音乐按钮总和
        """if self.is_mouse_over():
            self.rect_area.x += 1"""
        ...
        for i in 音乐按钮总和:
            i.update()
        # 绘制到底了文字
        self.image_area.blit(self.music_end_text, ((self.rect_panel.width - self.music_end_text.get_width())/2, 音乐按钮总和[-1].rect_item.y + 80))
        screen.blit(self.image_panel, self.rect_panel)
        if type(self.当前播放音乐名称) == str:
            self.music_name = self.font.render("正在播放：" + self.当前播放音乐名称.split(".")[0], True, (0, 0, 0))
            screen.blit(self.music_name, (width * 0.25 + 50, height - 100))
...
class Music_pay(Music_Panel):
    def __init__(self, x, y):
        super().__init__()
        self.x = x
        self.y = y
        # pygame.draw.rect(screen, (255, 255, 255), (width * 0.25, height - 150, width * 0.74, 130), border_radius=30)
        self.image_pay = pygame.Surface((width * 0.74, 130), pygame.SRCALPHA)
        self.rect_pay = self.image_pay.get_rect()
        self.rect_pay.x = self.x
        self.rect_pay.y = self.y
        self.image_pay.fill((255, 255, 255, 0))

        pygame.draw.rect(self.image_pay, (255, 255, 255), (0, 0, self.rect_pay.width, self.rect_pay.height), border_radius=30)
    def update(self):
        screen.blit(self.image_pay, self.rect_pay)

music_pay = Music_pay(music_area.rect_panel.x, music_area.rect_area.height + 40)
class Music_Button:
    def __init__(self, music_name, center_y, color, image_area, height):
        self.image_area = image_area
        self.height = height
        self.rect_area = self.image_area.rect_area
        self.center_y = center_y
        self.image_item = pygame.Surface((width * 0.7, self.height))
    ...
    def is_mouse_over(self):
        self.mouse_pos = pygame.mouse.get_pos()
        self.relative_pos = (
            self.mouse_pos[0] - (self.image_area.rect_panel.x + self.image_area.rect_area.x),
            self.mouse_pos[1] - (self.image_area.rect_panel.y + self.image_area.rect_area.y)
        )
...
for i in range(len(音乐总和_name)):
    ...
    音乐按钮总和.append(Music_Button(音乐总和_name[i], 60 * i + 35, color_music, music_area, 60))
def 听音乐screen():
    ...
    music_area.update()
    music_pay.update()
    # screen.blit(music_pay.image_pay, music_pay.rect_pay)
...
def 播放音乐():
    ...
    for i in range(len(音乐按钮总和)):
        if 音乐按钮总和[i].is_mouse_over() and music_area.is_mouse_over():
            if music_area.当前播放音乐 != None:
                music_area.当前播放音乐.stop()
            try:
                music_area.当前播放音乐 = pygame.mixer.Sound("../music/" + 音乐按钮总和[i].music_name)
                music_area.当前播放音乐名称 = 音乐按钮总和[i].music_name
                music_area.当前播放音乐.play()
            except:
                music_area.当前播放音乐名称 = "该文件不是音频格式"
        ...
        if event.type == pygame.QUIT:
            ...
            if 当前关卡() == 2 and event.button == 1:
                if clock1.is_mouse_over():
                    ...
                    clock3.可滑动 = True
            if 当前关卡() == 4:
                if event.button == 4 and 音乐按钮总和[0].rect_item.y < 5:
                    music_area.上滑动()
                elif event.button == 5 and 音乐按钮总和[-1].rect_item.y > music_area.rect_area.height - 150:
                    music_area.下滑动()
